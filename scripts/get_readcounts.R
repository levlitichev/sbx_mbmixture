...
if (exists("snakemake")) {
    ...
    option_list <- list( 
        ...
        make_option(c("-s", "--snp_db_path"), default="cc_variants.sqlite", metavar="path",
            help="Path to sqlite database containing variants for Collaborative Cross variants (can be downloaded from https://figshare.com/ndownloader/files/18533342) [default %default]"),
    ...
    args <- parse_args(OptionParser(
        description="For every non-complex SNP that can be used to distinguish one Collaborative Cross (CC) founder line from another, count how many reads in a given bam file support calling each of the two alleles.",
...
for(chr in CHROMOSOMES) {
  ...
  # TODO: Currently, a read can vote more than once at a homozygous position. That is, the read gets counted for both the positive and negative strand. Not sure how to get around this.
  ...
  # only keep pileup results for non-complex CC SNPs
  pileup.res <- pileup.res[pileup.res$pos %in% CC_snp_info$pos,,drop=FALSE]
  
  stopifnot(nrow(pileup.res) > 0)
  cat("Found", sum(pileup.res$count), "read counts at", length(unique(pileup.res$pos)), "(non-complex) CC SNPs\n")
  ...
  # for every non-complex CC SNP, count how many reads were found for each of the two alleles 
  ...
  # add to output list
  list.of.results[[chr]] <- out.counts.df
}
...
# write output
out.df <- do.call("rbind", list.of.results)
write.csv(out.df, args$out_path, quote=F, row.names=F)
